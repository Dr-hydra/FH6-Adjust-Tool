...
3. **本地方案存档与分享**
   ...
   - 支持只读扫描本地 FH6 存档中的 `Tuning_*` 调校容器，提取作者、调校名、车型和已识别的调校滑块，勾选后选择性导入为本地方案。
...
- **调校列表交互修复**：修复调校卡片初始半展开、展开后无法正常收起的问题；方案列表现在默认收起，展开/收起状态与内容高度同步更稳定。
- **批量删除联动优化**：进入或退出批量删除时不再重建列表，避免打断当前浏览；点击删除勾选框不会误触发卡片展开或收起。
- **本地存档导入增强**：支持扫描 FH6 本地 `Tuning_*` 存档容器，按车型筛选并选择性导入可识别调校。
- **方案列表检索与摘要**：保存的调校支持按车型筛选，列表卡片展示方案名、车型、性能等级与保存时间，展开后可继续编辑、覆盖、另存、分享或加载到主页。
- **遥测与圈速分析完善**：新增 WebView2 遥测仪表盘、路线轨迹、圈速记录、问题标记和调校对比，便于复盘不同调校方案的实际表现。
